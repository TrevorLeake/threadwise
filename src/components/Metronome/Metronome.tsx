...
const MetronomeWrapper = styled.footer`
  margin-top: 5rem;
  padding: 2rem 1rem;
  text-align: center;
  font-size: 0.9rem;
...
const MetronomeName = styled.div`
...
const MetronomeDate = styled.time`
  display: block;
  margin-top: 0.5rem;
  font-size: 0.85rem;
  opacity: 0.6;
...
export const Metronome = (props:{style?: CSSProperties}) => {
  ...
  return (
    <div data-testid='Metronome' className={styles.Metronome}>
        <audio ref={audioRef} hidden src='/bubble.mp3'></audio>
        <button onClick={() => {togglePlay()}}>START SCRIPT</button>
